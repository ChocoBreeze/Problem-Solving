...
int main() {
    ...
    const int SIZE = n + 1;
    vector<ll> pSum(SIZE, 0), dp(SIZE, 0);
    stk.assign(SIZE, lin());
    for (int i = 1; i <= n; ++i) {
        int x{}; cin >> x;
        pSum[i] = pSum[i - 1] + x;
    ...
    /*
        x = pSum[i] (변수)
        a = (-2 * a * pSum[j]) (기울기)
        b = a * {pSum[j]}^2 + DP[j] (절편)
        c = c + a * pSum[i]^2 + b * pSum[i] (상수)
    */

    for (int i = 1; i <= n; ++i) {
        dp[i] = c + a * pSum[i] * pSum[i] + b * pSum[i] + sol(pSum[i]); // 주어진 x좌표를 포함하는 선분(fpos)을 이분 탐색으로 찾음
        l.a = -2LL * a * pSum[i];
        l.b = dp[i] - b * pSum[i] + a * pSum[i] * pSum[i];
        insert(l);
    }
    cout << dp[n] << '\n';
    return 0;
